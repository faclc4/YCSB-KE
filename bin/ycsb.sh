...
export YCSB_OPTS="-Djava.net.preferIPv4Stack=true"
...
CLASSPATH="$CLASSPATH:db/tranHbase/conf"
echo $CLASSPATH
#echo "Executing command $CLASS with options $JAVA_HEAP_MAX $YCSB_OPTS $CLASS $@"
exec "$JAVA" $JAVA_HEAP_MAX -Dlog4j.configuration=log4j.properties $YCSB_OPTS -classpath "$CLASSPATH" $CLASS "$@"
